...
### Fixed
- Searching your work orders from a connected assistant returned an error
  instead of results. The previous release said this was fixed; duplicate
  detection was, but search was not. It works now. No action is needed
  beyond upgrading.
...
### Fixed
- The container image now appears in this repository's package listing. It was always pullable, but it was filed under a different repository, so browsing here showed no image. Nothing about the image itself changed and no action is required.
...
- The Helm chart installs directly from the registry, with no download step:
  ...
  installs, and each release verifies that both copies are byte-identical.
