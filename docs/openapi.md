...
- Path items: `117`.
- Literal server registrations: `147` — `75 GET`, `46 POST`, `12 PUT`, `10 PATCH`, and `4 DELETE`.
- Effective HTTP operations: `222`, including the `75 HEAD` operations supplied by Go HTTP routing for registered `GET` routes.
- Lifecycle-verified operations: `11`; their request and response schemas are exercised against real handlers.
- Route-only-unverified operations: `211`; literal paths and HTTP methods are registration-verified, while request and response bodies are not handler-verified.
